...
class CodeLike:
    ...
    # the semantic meaning of this wrap func differs from that of others
    @staticmethod
...
class ActorPromptFactory(AIOPromptFactory):
    # first section: _intro
    GENERAL_INTRO = "You are an agent which follow my instruction and assist to perform desktop computer tasks as instructed."
    OBS_INCENTIVE = lambda obs_descr: f"For each step, you will get an observation of the desktop by {obs_descr}, together with a next-step action generated by another agent, and you will parse the placeholder of `{CoderPromptFactory.PLACEHOLDER}` to make the script runable."

    # second section: _command
    RETURN_OVERVIEW_VM = AIOPromptFactory.RETURN_OVERVIEW_VM.copy()
    RETURN_OVERVIEW_VM = {
        "gui_actor": f"You will receive a code blocks with the placeholder, like this:\n```\n# click the shield icon\n{CoderPromptFactory.PLACEHOLDER}\n```"
    }
    RETURN_REGULATION = AIOPromptFactory.RETURN_REGULATION.copy()
    RETURN_REGULATION.update({
        "gui_actor": "You ONLY need to return a json object recording the coordinate of the target element described in the comment above, like this:\n{\"xy\": [0.27, 0.48]}"
    })
    RETURN_SUPPLEMENT_VM = AIOPromptFactory.RETURN_SUPPLEMENT_VM.copy()
    RETURN_SUPPLEMENT_VM.update({
        "gui_actor": ""
